...
const getMinimistOpts = questions => questions.reduce((m, question) => {
...
const processPositionalArgs = (questions = [], argv = {
  _: []
}) => {
  const _questions = questions.filter(q => q.hasOwnProperty('_') && typeof argv[q.name] === 'undefined');

  const types = _questions.map(q => q.type);

  const arrayOccurences = types.reduce((m, v) => v === 'array' ? m + 1 : m, 0);
  ...
  const args = types.map(q => q === 'array' ? Math.max(1, argLength - len) : 1);
  const ranges = args.reduce((m, v, i) => i === 0 ? m.concat([[0, v]]) : m.concat([[m[i - 1][1], m[i - 1][1] + v]]), []);
  const max = ranges.length ? ranges[ranges.length - 1][1] : 0; // if (max < argv._.length)  /* too many positional arguments */
...
const processFilteredArgs = (questions = [], argv = {
  _: []
}) => {
...
const processArrayArgs = (questions = [], argv = {
  _: []
}) => {
  //  arrays here
  questions.filter(q => q.type === 'array').forEach(question => {
    argv[question.name] = argv[question.name] instanceof Array ? argv[question.name] : [argv[question.name]];
  });
  return argv;
};

const processDefaultArgs = (questions = [], argv = {
  _: []
}) => {
  //  defaults here
  questions.filter(q => typeof q.default !== 'undefined' || q.type === 'boolean').map(q => typeof q.default !== 'undefined' ? q : _objectSpread({}, q, {
    default: false
  })).forEach(question => {
...
const processArgs = (questions = [], argv = {
  _: []
}) => {
...
const validateArgs = (questions = [], argv = {
  _: []
}) => {
  questions.filter(q => q.required).forEach(question => {
...
const processArgPaths = (questions = [], argv = {
  _: []
}) => {
  questions.filter(q => q.type === 'path' || q.type === 'array' && q.items.type === 'path').forEach(question => {
...
export { getMinimistOpts, processArgs, processArgPaths, validateArgs, processArrayArgs, processFilteredArgs, processPositionalArgs };